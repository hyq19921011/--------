...
void AD_init()//10位
{
	P1ASF|=0x0c;       //将P1^2口和1^3作为ADC功能使用
	CLK_DIV&=~0x20;      //0000,0100 令 ADRJ=1，10位A/D转换结果的高2位存放在ADC_RES的低2位中，低8位存放在ADC_RESL中
	ADC_CONTR = ADC_POWER | ADC_SPEEDH; //1110,0000 令 ADC_POWER=1，将ADC电源打开， SPEED1，SPEED0：模数转换器转换速度控制位
...
WORD Get_ad_result(BYTE channel)
{
	...
	{
		ADC_CONTR |= ADC_POWER | ADC_SPEEDH | channel | ADC_START;   //0000,1xxx 令 ADC_START=1, 启动A/D 转换 ;选择模拟信号输入IO通道
